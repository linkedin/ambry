...
public class ByteBufferReadableStreamChannelParameterizedTest {
  public static final int BUFFER_SIZE = 256;

  private ByteBuffer contentWrapper;
  private ByteBufferReadableStreamChannel byteBufferReadableStreamChannel;

  public ByteBufferReadableStreamChannelParameterizedTest(List<ByteBuffer> bufferList, ByteBuffer content1,
      ByteBuffer content2) {
    if (bufferList.size() == 1) {
      byteBufferReadableStreamChannel = new ByteBufferReadableStreamChannel(bufferList.get(0));
      contentWrapper = ByteBuffer.wrap(content1.array());
    } else {
      byteBufferReadableStreamChannel = new ByteBufferReadableStreamChannel(bufferList);
      contentWrapper = ByteBuffer.allocate(BUFFER_SIZE * 2);
      contentWrapper.put(content1);
      contentWrapper.put(content2);
      content1.rewind();
      content2.rewind();
  ...
  public static List<Object[]> data() {
    ByteBuffer content1 = ByteBuffer.wrap(fillRandomBytes(new byte[BUFFER_SIZE]));
    ByteBuffer content2 = ByteBuffer.wrap(fillRandomBytes(new byte[BUFFER_SIZE]));
    ByteBuffer content3 = ByteBuffer.wrap(fillRandomBytes(new byte[BUFFER_SIZE]));

    // @formatter:off
    return Arrays.asList(new Object[][] {
        {Arrays.asList(content1), content1, null},
        {Arrays.asList(content2, content3), content2, content3}
    });
    // @formatter:on
  }
   ...
   */
  private static byte[] fillRandomBytes(byte[] in) {
