...
public class ReplicationTest {
  ...
  public void replicaThreadTest()
    ...
    try {
      ...
      DataNodeId firstDataNodeId = clusterMap.getDataNodeIds().get(0);
      DataNodeId secondDataNodeId = clusterMap.getDataNodeIds().get(1);

      List<ReplicaId> replicaIds =
          clusterMap.getReplicaIds(clusterMap.getDataNodeId("localhost", firstDataNodeId.getPort()));
      ...
      replicaStores.put("localhost" + secondDataNodeId.getPort(), messageInfoNode2);
      replicaBuffers.put("localhost" + secondDataNodeId.getPort(), bufferListNode2);
      ...
      for (RemoteReplicaInfo remoteReplicaInfo : remoteReplicas) {
        if (remoteReplicaInfo.getReplicaId().getDataNodeId().getPort() == secondDataNodeId.getPort()) {
      ...
      ReplicaThread replicaThread =
          new ReplicaThread("threadtest", replicasToReplicate, new MockFindTokenFactory(), clusterMap,
              new AtomicInteger(0), clusterMap.getDataNodeId("localhost", firstDataNodeId.getPort()),
      ...
      List<ReplicaThread.ExchangeMetadataResponse> response = replicaThread.exchangeMetadata(
          new MockConnection("localhost", secondDataNodeId.getPort(),
              replicaStores.get("localhost" + secondDataNodeId.getPort()),
              replicaBuffers.get("localhost" + secondDataNodeId.getPort()), 5), replicasToReplicate.get(dataNodeId),
          false, false);
      ...
      response = replicaThread.exchangeMetadata(new MockConnection("localhost", secondDataNodeId.getPort(),
          replicaStores.get("localhost" + secondDataNodeId.getPort()),
          replicaBuffers.get("localhost" + secondDataNodeId.getPort()), 5), replicasToReplicate.get(dataNodeId), false,
          false);
      Assert.assertEquals(response.size(), replicasToReplicate.get(dataNodeId).size());
      for (int i = 0; i < response.size(); i++) {
        Assert.assertEquals(response.get(i).missingStoreKeys.size(), 0);
      ...
      response = replicaThread.exchangeMetadata(new MockConnection("localhost", secondDataNodeId.getPort(),
          replicaStores.get("localhost" + secondDataNodeId.getPort()),
          replicaBuffers.get("localhost" + secondDataNodeId.getPort()), 4), replicasToReplicate.get(dataNodeId), false,
          false);
      Assert.assertEquals(response.size(), replicasToReplicate.get(dataNodeId).size());
      for (int i = 0; i < response.size(); i++) {
      ...
      replicaThread.fixMissingStoreKeys(new MockConnection("localhost", secondDataNodeId.getPort(),
          replicaStores.get("localhost" + secondDataNodeId.getPort()),
          replicaBuffers.get("localhost" + secondDataNodeId.getPort()), 4), replicasToReplicate.get(dataNodeId), false,
          response, false);
      for (int i = 0; i < response.size(); i++) {
