...
public abstract class ReplicaTokenPersistor implements Runnable {
  ...
  public static class ReplicaTokenSerde {
    ...
    // Map<Sting,FindToken>
    public ReplicaTokenSerde(ClusterMap clusterMap, FindTokenFactoryFactory findTokenFactoryFactory) {
        ...
        switch (version) {
          case 0:
            while (stream.available() > Crc_Size) {
              ...
              FindTokenFactory findTokenFactory = findTokenFactoryFactory.getFindTokenFactoryFromStream(peekableInputStream);
              FindToken token = findTokenFactory.getFindToken(peekableInputStream);
              tokenInfoList.add(
