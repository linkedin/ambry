...
public class ReplicationPrioritizationMetrics {
  ...
  /**
   * Get a timer context for measuring prioritization cycle time.
   * @return Timer.Context to measure time.
   */
  public Timer.Context startPrioritizationCycleTimer() {
    return prioritizationCycleTime.time();
  }

  /**
   * Get a timer context for measuring fetch disruptions time.
   * @return Timer.Context to measure time.
   */
  public Timer.Context startFetchDisruptionsTimer() {
    return fetchDisruptionsTime.time();
  }

  /**
   * Get a timer context for measuring control replication threads time.
   * @return Timer.Context to measure time.
   */
  public Timer.Context startControlReplicationThreadsTimer() {
    return controlReplicationThreadsTime.time();
  }

  /**
   * Updates metrics for a completed prioritization run.
   * @param newHighPriorityPartitions Any new high priority partitions identified
   * @param completedPartitions Partitions that completed replication
   * @param wasNormalToHighPriorityTransition Whether this run switched from normal to high priority mode
   */
  public void updatePrioritizationRunMetrics(
      Set<PartitionId> newHighPriorityPartitions,
      Set<PartitionId> completedPartitions,
      boolean wasNormalToHighPriorityTransition) {

    if (!newHighPriorityPartitions.isEmpty()) {
      recordPartitionsAddedToHighPriority(newHighPriorityPartitions);
    }

    for (PartitionId partitionId : completedPartitions) {
      // For simplicity, we're not tracking actual replication time here
      // In practice, you'd want to capture the real replication duration
      recordPartitionReplicationComplete();
    }

    if (wasNormalToHighPriorityTransition) {
      recordNormalToHighPriorityTransition();
    }
  }
}