...
public class StoreMetrics {
  ...
  public StoreMetrics(String replicaDir, MetricRegistry registry) {
    this.registry = registry;
    name = getPartitionName(replicaDir);
  ...
  static String getPartitionName(String replicaDir) {
    if (replicaDir.charAt(replicaDir.length() - 1) == '/') {
      replicaDir = replicaDir.substring(0, replicaDir.length() - 1);
    }
    return replicaDir.substring(replicaDir.lastIndexOf('/') + 1);
  }
}
