...
public class PersistentIndex {
  ...
  public PersistentIndex(String datadir, Scheduler scheduler, Log log, StoreConfig config, StoreKeyFactory factory,
    ...
    try {
      ...
      // schedule the cleanup thread via the thread pool, but not as a periodic task. Every invocation will determine
      // when the next invocation needs to happen.
      this.scheduler.schedule("cleanup thread" + dataDir, cleanupThread,
          config.storeDataFlushDelaySeconds + new Random().nextInt(SystemTime.SecsPerMin), -1, TimeUnit.SECONDS);
  ...
  class CleanupThread implements Runnable {
    ...
    FindToken startTokenBeforeFlush;
    ...
    private void performRecovery()
          ...
          switch (version) {
            ...
            default:
              throw new StoreException("Invalid version in cleanup token " + dataDir,
                  StoreErrorCodes.IOError); //@TODO change the error
          }
          long crc = crcStream.getValue();
          if (crc != stream.readLong()) {
            throw new StoreException("Crc check does not match for cleanup token file for dataDir " + dataDir,
                StoreErrorCodes.IOError); //@TODO error
          }
        } catch (IOException e) {
          throw new StoreException("Failed to read cleanup token", e, StoreErrorCodes.IOError); //@TODO change the error
        } finally {
          stream.close();
        }
      } else {
      ...
      startTokenBeforeFlush = startTokenSafeToPersist = startToken;
      // perform recovery
      hardDelete();
    }
    ...
    private void performHardDeletes(List<MessageInfo> messageInfoList)
      ...
      try {
        ...
        logger.error("Exception"); //@TODO: remove try-catch
      }
    }

    /* Scans a set of ids from the index and performs hard deletion
       1. Start at the current token S.
       2. E, entries = findDeletedEntriesSince(start_point).
       4. Persist E.
       5. performHardDelete(entries) // this is going on for [S, E) //take care of off by 1 errors.
       6. set S' = S, S = E
       7. Persistor runs in the background flushes S' after the log is flushed.
     */
    ...
    public void run() {
      try {
        ...
        //TODO: throlling logic needs some work.
        ...
        scheduler.schedule("cleanup thread" + dataDir, this, 0, 0, TimeUnit.SECONDS);
