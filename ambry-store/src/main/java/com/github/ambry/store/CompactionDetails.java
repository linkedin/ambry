...
class CompactionDetails {
  ...
  private static final int IS_FULL_RANGE_SIZE = 1;
  ...
  CompactionDetails(long referenceTimeMs, List<LogSegmentName> logSegmentsUnderCompaction,
    ...
    this(referenceTimeMs, logSegmentsUnderCompaction, costBenefitInfo, false);
  }
   ...
   * @param isFullRange {@code True} if this CompactionDetails are generated from a {@link CompactAllPolicy}.
   * @throws IllegalArgumentException if {@code referenceTimeMs} < 0 or if {@code logSegmentsUnderCompaction} has no
   * elements.
   */
  CompactionDetails(long referenceTimeMs, List<LogSegmentName> logSegmentsUnderCompaction,
      CostBenefitInfo costBenefitInfo, boolean isFullRange) {
  ...
  static CompactionDetails fromBytes(DataInputStream stream) throws IOException {
    ...
    switch (version) {
      ...
      case VERSION_1:
        ...
        details = new CompactionDetails(referenceTime, logSegmentsUnderCompaction, null, isFullRange);
        break;
      default:
  ...
  /**
   * @return {@code True} if this CompactionDetails was generated by a {@link CompactAllPolicy}.
   */
  boolean isFullRange() {
    return isFullRange;
  }
  ...
  byte[] toBytes() {
    ...
    int size =
        VERSION_SIZE + REFERENCE_TIME_SIZE + SEGMENT_COUNT_SIZE + (CURRENT_VERSION == VERSION_1 ? IS_FULL_RANGE_SIZE
            : 0);
    ...
    // version
    bufWrap.putShort(CURRENT_VERSION);
    // reference time
    bufWrap.putLong(referenceTimeMs);
    if (CURRENT_VERSION == VERSION_1) {
      bufWrap.put(isFullRange ? (byte) 1 : (byte) 0);
    }
