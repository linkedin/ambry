 ...
 */
public class ServerRequestResponseHelper {

  private final ClusterMap clusterMap;
  private final FindTokenHelper findTokenHelper;

  public ServerRequestResponseHelper(ClusterMap clusterMap, FindTokenHelper findTokenHelper) {
  ...
  public Response createErrorResponse(RequestOrResponse request, ServerErrorCode serverErrorCode) {
    ...
    switch (type) {
      ...
      case FileCopyGetChunkRequest:
        response = new FileCopyGetChunkResponse(request.getCorrelationId(), request.getClientId(), serverErrorCode);
        break;
      default:
        throw new UnsupportedOperationException("Request type not supported");
    }
