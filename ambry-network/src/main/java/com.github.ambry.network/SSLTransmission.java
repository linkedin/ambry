...
public class SSLTransmission extends Transmission implements ReadableByteChannel, WritableByteChannel {
  ...
  public boolean read() throws IOException {
    ...
    if (bytesRead > 0) {
      metrics.sslReceiveTimeInUsPerKB.update(readTimeMs * 1024000 / bytesRead);
    }
    ...
    if (dst.remaining() > 0) {
      ...
      do {
        ...
        if (unwrapResult.bytesProduced() > 0) {
          metrics.sslDecryptionTimeInUsPerKB.update(decryptionTimeMs * 1024000 / unwrapResult.bytesProduced());
        }
        netReadBuffer.compact();
  ...
  public boolean write() throws IOException {
    ...
    if (bytesWritten > 0) {
      metrics.sslSendTimeInUsPerKB.update(writeTimeMs * 1024000 / bytesWritten);
      metrics.sslSendTimeInUs.update(writeTimeMs * 1000);
    }
  ...
  public int write(ByteBuffer src) throws IOException {
    ...
    if (wrapResult.bytesConsumed() > 0) {
      metrics.sslEncryptionTimeInUsPerKB.update(encryptionTimeMs * 1024000 / wrapResult.bytesConsumed());
    }
    netWriteBuffer.flip();
