...
public class AmbryServer {
  ...
  private final VcrClusterAgentFactory vcrClusterAgentFactory;
  ...
  public AmbryServer(VerifiableProperties properties, ClusterAgentsFactory clusterAgentsFactory,
      VcrClusterAgentFactory vcrClusterAgentFactory, Time time) throws InstantiationException {
    this(properties, clusterAgentsFactory, vcrClusterAgentFactory, new LoggingNotificationSystem(), time, null);
  }
   ...
   * @param vcrClusterAgentFactory a {@link VcrClusterAgentFactory} instance. Required if replicating from a VCR.
   ...
   */
  public AmbryServer(VerifiableProperties properties, ClusterAgentsFactory clusterAgentsFactory,
      VcrClusterAgentFactory vcrClusterAgentFactory, NotificationSystem notificationSystem, Time time,
    ...
    this.vcrClusterAgentFactory = vcrClusterAgentFactory;
  ...
  public void startup() throws InstantiationException {
    try {
      ...
      if (replicationConfig.replicationEnabledWithVcrCluster) {
        ...
        vcrClusterSpectator = vcrClusterAgentFactory.getVcrClusterSpectator(cloudConfig, clusterMapConfig);
      ...
      accountStatsMySqlStore =
          statsConfig.enableMysqlReport ? (AccountStatsMySqlStore) new AccountStatsMySqlStoreFactory(properties,
              clusterMapConfig, statsConfig, registry).getAccountStatsStore() : null;
