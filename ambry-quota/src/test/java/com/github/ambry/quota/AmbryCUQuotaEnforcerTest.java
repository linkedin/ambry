...
public class AmbryCUQuotaEnforcerTest {
  ...
  public void chargeAndRecommendTest() throws Exception {
    ...
    Map<String, JsonBackedCUQuotaSource.CUQuota> usageMap = QUOTA_SOURCE.getAllQuotaUsage();
    ...
    QuotaRecommendation quotaRecommendation =
        AMBRY_QUOTA_ENFORCER.chargeAndRecommend(QuotaTestUtils.createRestRequest(ACCOUNT, container, RestMethod.GET),
            blobInfo, readRequestCostMap);
    ...
    assertEquals(usageMap.get(String.valueOf(ACCOUNT.getId())).getWcu(), 0, 0.01); // make sure that correct quota is charged.

    quotaRecommendation =
        AMBRY_QUOTA_ENFORCER.chargeAndRecommend(QuotaTestUtils.createRestRequest(ACCOUNT, container, RestMethod.POST),
            blobInfo, writeRequestCostMap);
    assertEquals(quotaRecommendation.getQuotaName(), QuotaName.WRITE_CAPACITY_UNIT);
    assertEquals(quotaRecommendation.getQuotaUsagePercentage(), 90, 0.1);
    assertEquals(quotaRecommendation.getRecommendedHttpStatus(), 200);
    assertEquals(quotaRecommendation.shouldThrottle(), false);
    assertEquals(usageMap.get(String.valueOf(ACCOUNT.getId())).getRcu(), 9, 0.01); // make sure that correct quota is charged.
    ...
    try {
      AMBRY_QUOTA_ENFORCER.chargeAndRecommend(
      ...
      Assert.assertFalse(quotaException.isRetryable());
    }
    ...
    assertEquals(9, usageMap.get(String.valueOf(ACCOUNT.getId())).getWcu(), 0.01); // make sure that correct quota is charged.
    assertEquals(18,
        usageMap.get(String.valueOf(ACCOUNT.getId())).getRcu(), 0.01); // make sure that correct quota is charged.
    ...
    assertEquals(true, quotaRecommendation.shouldThrottle());
    assertEquals(18,
        usageMap.get(String.valueOf(ACCOUNT.getId())).getWcu(), 0.01); // make sure that correct quota is charged.
    assertEquals(18,
        usageMap.get(String.valueOf(ACCOUNT.getId())).getRcu(), 0.01); // make sure that correct quota is charged.

    quotaRecommendation =
        AMBRY_QUOTA_ENFORCER.chargeAndRecommend(QuotaTestUtils.createRestRequest(ACCOUNT, container, RestMethod.DELETE),
            blobInfo, writeRequestCostMap);
    assertEquals(QuotaName.WRITE_CAPACITY_UNIT, quotaRecommendation.getQuotaName());
    ...
    quotaRecommendation =
        AMBRY_QUOTA_ENFORCER.chargeAndRecommend(QuotaTestUtils.createRestRequest(ACCOUNT, container, RestMethod.PUT),
            blobInfo, writeRequestCostMap);
    assertEquals(QuotaName.WRITE_CAPACITY_UNIT, quotaRecommendation.getQuotaName());
  ...
  public void getResourceRecommendationTest() throws Exception {
    ...
    Map<String, JsonBackedCUQuotaSource.CUQuota> usageMap = QUOTA_SOURCE.getAllQuotaUsage();
    Map<String, JsonBackedCUQuotaSource.CUQuota> quotaMap = QUOTA_SOURCE.getAllQuota();
    String id = String.valueOf(ACCOUNT.getId());
    usageMap.put(id,
        new JsonBackedCUQuotaSource.CUQuota((long) quotaMap.get(id).getRcu() + 1, (long) quotaMap.get(id).getWcu()));
  ...
  public void isQuotaExceedAllowedTest() throws Exception {
    ...
    QUOTA_SOURCE.setFeUsage((long) QUOTA_SOURCE.getFeQuota().getRcu(), (long) QUOTA_SOURCE.getFeQuota().getWcu());
    ...
    String id = String.valueOf(ACCOUNT.getId());
    usageMap.put(id,
        new JsonBackedCUQuotaSource.CUQuota((long) quotaMap.get(id).getRcu() + 1, (long) quotaMap.get(id).getWcu() + 1));
  ...
  static class ExceptionQuotaSource extends JsonBackedCUQuotaSource {
