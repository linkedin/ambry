...
public class BlobOutput implements ReadableChannel {
  ...
  /**
   * Gets the stream that contains blob data. If called after {@link #open()} is called, the received stream will
   * behave unpredictably.
   * @return stream that contains blob data.
   */
  ...
  /**
   * Opens BlobOutput as a ReadableChannel. Once open is called, manipulating the stream received through
   * {@link #getStream()} will cause {@link #read(ByteBuffer)} and {@link #writeTo(WritableByteChannel)} to behave
   * unpredictably.
   * @return This BlobOutput
   * @throws IllegalStateException if open has been called before
   * @throws ClosedChannelException if channel has already been closed (i.e. {@link #close()} was called)
   * @throws IOException if data from the backing {@link InputStream} could not be read or if the size read did not
   * match the reported size of the stream.
   */
  public BlobOutput open()
      throws IOException {
