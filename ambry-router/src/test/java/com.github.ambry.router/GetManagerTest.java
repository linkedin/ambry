...
public class GetManagerTest {
  ...
  public void testRangeRequest()
      throws Exception {
    testGetSuccess(chunkSize * 6 + 11, new GetBlobOptions(GetBlobOptions.OperationType.Data, GetOptions.None,
  ...
  private void testGetSuccess(int blobSize, GetBlobOptions options)
    ...
    this.options = new GetBlobOptions(GetBlobOptions.OperationType.BlobInfo, GetOptions.None, null);
    getBlobAndCompareContent(blobId);
    router.close();
  }
  ...
  private void testBadCallback(Callback<GetBlobResult> getBlobCallback, CountDownLatch getBlobCallbackCalled,
    ...
    GetBlobOptions infoOptions = new GetBlobOptions(GetBlobOptions.OperationType.BlobInfo, GetOptions.None, null);
    GetBlobOptions dataOptions = new GetBlobOptions(GetBlobOptions.OperationType.Data, GetOptions.None, null);
    for (int i = 0; i < 5; i++) {
      if (i == 1) {
  ...
  public void testFailureOnAllPollThatSends()
    ...
    Future future;
    try {
      future = router.getBlob(blobId, new GetBlobOptions(GetBlobOptions.OperationType.BlobInfo, GetOptions.None, null));
      while (!future.isDone()) {
