...
public class OperationController implements Runnable {
   ...
   * @param isUserRequest {@code true} when delete is requested by a user. {@code false} if the delete request is
  ...
  protected void deleteBlob(final String blobIdStr, final String serviceId, FutureResult<Void> futureResult,
    ...
    try {
      deleteManager.submitDeleteBlobOperation(blobIdStr, serviceId, futureResult,
          ...
          }, quotaChargeCallback, isUserRequest);
    } catch (RouterException e) {
...
class BackgroundDeleter extends OperationController {
  ...
  @Override
  protected void deleteBlob(final String blobIdStr, final String serviceId, FutureResult<Void> futureResult,
      final Callback<Void> callback, boolean attemptChunkDeletes, QuotaChargeCallback quotaChargeCallback, boolean isUser) {
    Supplier<Void> deleteCall = () -> {
      ...
      }, attemptChunkDeletes, quotaChargeCallback, false);
      return null;
    };
