...
public class NamedBlobsCleanupRunnerTest {
  ...
  @Test
  public void testAccountLevelExclusionSkipsEveryContainerInTheAccount() {
    // A bare "accountName" entry must exempt every container in that account, while a container in a different
    // account is still cleaned.
    ...
    when(excludedAccount.getName()).thenReturn("excluded-account");
    Account otherAccount = mock(Account.class);
    when(otherAccount.getName()).thenReturn("other-account");
    AccountService accountService = mock(AccountService.class);
    ...
    new NamedBlobsCleanupRunner(mock(Router.class), namedBlobDb, accountService, 0,
        Collections.singleton("excluded-account"), new MockTime()).run();
