...
// postGetHeadUpdateDeleteTest() and multipartPostGetHeadUpdateDeleteTest() helpers
  ...
  List<Dataset> doDatasetPutUpdateGetTest(Account account, Container container) throws Exception {
    String accountName = account.getName();
    String containerName = container.getName();
    ...
    for (Dataset.VersionSchema versionSchema : versionSchemas) {
      ...
      Dataset dataset =
          new DatasetBuilder(accountName, containerName, datasetName).setVersionSchema(versionSchema).build();
      HttpHeaders headers = new DefaultHttpHeaders();
      //Test put dataset
  ...
  List<Pair<String, String>> doDatasetVersionPutGetTest(Account account, Container container, List<Dataset> datasets,
    ...
    for (Dataset dataset : datasets) {
      for (long ttl : new long[]{-1, TTL_SECS}) {
        ...
        getDatasetVersionInfoAndVerify(dataset, version, getHeaders, ttl, expectedGetHeaders);
  ...
  void getDatasetVersionInfoAndVerify(Dataset dataset, String version, HttpHeaders headers, long expectTtl,
      HttpHeaders expectedHeaders) throws Exception {
    String accountName = dataset.getAccountName();
    String containerName = dataset.getContainerName();
    String datasetName = dataset.getDatasetName();
